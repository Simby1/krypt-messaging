...
function extractPayload(raw: any) {
  try {
    let p = raw;
    for (let i = 0; i < 3; i++) {
      if (typeof p === "string") { try { p = JSON.parse(p); } catch { break; } }
    }
...
export default function ChatPage() {
  ...
  // Refs so WS callback always sees latest values without re-registering
  ...
  useEffect(() => {
    ...
    const pk  = localStorage.getItem("wb_public_key") || "";
    setMyUserId(uid);
    setMyPublicKey(pk);
    myUserIdRef.current    = uid;
    myPublicKeyRef.current = pk;
    setDisplayName(localStorage.getItem("wb_display_name") || "");
  }, [router]);

  // ─── Conversations ─────────────────────────────────────────────────────────
  // Defined before WS handler so it can be referenced in the ref below
  ...
  // Stable ref so WS callback can call it without stale closure
  ...
  // ─── Decrypt helpers ───────────────────────────────────────────────────────
  const decryptOne = useCallback(async (msg: Message, isMine: boolean): Promise<Message> => {
    try {
  ...
  // Stable ref for WS callback
  const decryptOneRef = useRef(decryptOne);
  useEffect(() => { decryptOneRef.current = decryptOne; }, [decryptOne]);

  // ─── WebSocket ─────────────────────────────────────────────────────────────
  // Uses refs only — never re-registers even if state changes
  const handleWsMessage = useCallback((data: any) => {
    ...
    if (event === "message.receive" && data.message) {
      ...
      if (convo && (msg.from_user_id === convo.user_id || msg.to_user_id === convo.user_id)) {
        decryptOneRef.current(msg, isMine).then((decrypted) => {
          setMessages((prev) => {
            // Deduplicate: skip if already present (real id or matching pending)
            if (prev.find((m) => m.id === decrypted.id)) return prev;
            // Replace matching optimistic pending message if text matches
            const pendingIdx = prev.findIndex(
              (m) => m._pending && m._text === decrypted._text && m.from_user_id === decrypted.from_user_id
            );
            if (pendingIdx !== -1) {
              const next = [...prev];
              next[pendingIdx] = decrypted;
              return next;
            }
  ...
  const transmit = async () => {
    ...
    try {
      ...
      const payload = await encryptMessage(text, recipientPublicKey, myPublicKeyRef.current);
      ...
      // Try WS for real-time delivery to recipient
      ...
      console.log("[send] WS:", wsSent);

      if (!wsSent) {
        // WS not connected — use REST as the sole delivery path
        await sendMessageRest(convo.user_id, payload);
        console.log("[send] REST ok (WS was down)");
      }
      // If WS succeeded, do NOT also call REST — that's what caused the duplicates

      // Confirm optimistic message
      setMessages((prev) =>
        prev.map((m) => m.id === optimisticId ? { ...m, _pending: false } : m)
      );
      loadConversations();
    } catch (e: any) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        setSearchResults(Array.isArray(r) ? r.filter((u: SearchResult) => u.id !== myUserIdRef.current) : []);
  ...
  return (
    <div className="flex h-screen h-[100dvh] bg-[#050505] text-zinc-300 font-sans overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className={`flex flex-col border-r border-white/5 bg-zinc-900/20 w-full md:w-72 md:flex shrink-0 ${showSidebar ? "flex" : "hidden"}`}>
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {conversations.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-3 px-6 text-center">
              ...
              <p className="text-xs text-zinc-700 leading-relaxed">No conversations yet.<br />Search for a user to start.</p>
            </div>
          ) : (
            conversations.map((c) => (
              <button key={c.user_id} onClick={() => openConversation(c)}
                className={`w-full flex items-center gap-3 px-4 py-3 transition-all text-left ${activeConvo?.user_id === c.user_id ? "bg-white/5" : "hover:bg-white/[0.03]"}`}>
      ...
      {/* Chat panel */}
      <main className={`flex-1 flex flex-col min-w-0 w-full md:flex ${showSidebar ? "hidden md:flex" : "flex"}`}>
        {activeConvo ? (
          <>
            <header className="h-14 border-b border-white/5 flex items-center justify-between px-4 md:px-6 bg-black/30 shrink-0 gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <button onClick={() => setShowSidebar(true)} className="md:hidden text-zinc-500 hover:text-zinc-300 transition-colors shrink-0 p-1 -ml-1">
                  <ArrowLeft size={20} />
                </button>
                    ...
                    <div key={m.id || i} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                      <div className="max-w-[80%] sm:max-w-xs lg:max-w-md space-y-1">
                        <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed break-words
                          ${isMine ? "bg-purple-600 text-white rounded-br-sm" : "bg-zinc-900/70 border border-white/5 text-zinc-200 rounded-bl-sm"}
